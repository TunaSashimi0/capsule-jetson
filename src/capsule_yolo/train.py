...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--imgsz", type=int, default=DEFAULT_MODEL_IMGSZ, help="Training image size.")
...
def main() -> None:
    ...
    # Ultralytics increments an existing run name (for example, ``name-2``).
    # Use the trainer's resolved checkpoint path instead of reconstructing it
    # from the requested name, which may no longer match the actual run folder.
    if model.trainer is not None:
        run_best = Path(model.trainer.best)
    else:
